...
class EditDeviceNetworkScreen extends StatefulWidget {
  ...
  @override
  _EditDeviceNetworkScreenState createState() => _EditDeviceNetworkScreenState();
}
    ...
    return BlocListener<DeviceBloc, DeviceState>(
      listener: (context, state) {
        ...
        if( state is LoadingDeviceState ){
          ...
          modalSuccess(context, 'Device\'s snetwork updated', () => Navigator.pop(context));
            ...
            TextButton(
              onPressed: (){
                if( _keyForm.currentState!.validate()){
                  deviceBloc.add( OnEditDeviceNetworkEvent( _deviceController.text, _nameController.text, _passwordController.text ));
                }
              }, 
              child: TextCustom(text: 'Update network', fontSize: 16, color: Colors.amber[900]!)
            )
          ],
        ),
        body: SafeArea(
          child: Form(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 20.0, vertical: 10.0),
              children: [
                const TextCustom(text: 'Device', color: ColorsEnum.secundaryColor),
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.hasData && snapshot.data!.isNotEmpty) {
                      if (_deviceController.text.isEmpty) {
                        _deviceController.text = snapshot.data!.first.deviceKey;
                      }
                      ...
                      return DropdownButtonFormField<String>(
                        ...
                        items: snapshot.data!.map<DropdownMenuItem<String>>((DeviceNetwork value) {
                ...
                const TextCustom(text: 'Name', color: ColorsEnum.secundaryColor),
                const SizedBox(height: 5.0),
                FormFieldFrave(
                ...
                const SizedBox(height: 20.0),
                const TextCustom(text: 'Password', color: ColorsEnum.secundaryColor),
                const SizedBox(height: 5.0),
                FormFieldFrave(
