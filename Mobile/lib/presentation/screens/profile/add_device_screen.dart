            ...
            TextButton(
              onPressed: (){
                if( _keyForm.currentState!.validate()){
                  deviceBloc.add( OnAddDeviceEvent( _keyController.text, _phoneController.text, _usernameController.text ));
                }
              }, 
              child: TextCustom(text: 'Save', fontSize: 16, color: Colors.amber[900]!)
            )
          ],
        ),
        body: SafeArea(
          child: Form(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 20.0, vertical: 10.0),
              children: [
                const TextCustom(text: 'Device key', color: ColorsEnum.secundaryColor),
                const SizedBox(height: 5.0),
                FormFieldFrave(
                  controller: _keyController,
                  validator: RequiredValidator(errorText: 'Device\'s key is required')
                ),
                const SizedBox(height: 20.0),
                const TextCustom(text: 'Phone', color: ColorsEnum.secundaryColor),
                const SizedBox(height: 5.0),
                FormFieldFrave(
                ...
                const SizedBox(height: 20.0),
                const TextCustom(text: 'Username', color: ColorsEnum.secundaryColor),
                const SizedBox(height: 5.0),
                FormFieldFrave(
